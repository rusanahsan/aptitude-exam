...
function fetchquestion(num){
    ...
    let temps="";
    if(num!=sectional_qnum[curr_sec][0])
        temps+=`<button type="button" class="btn btn-outline-primary" id="Prev" onClick="prevclick()">Back</button>`;
    if(curr_sec!=secnum-1)
        temps+=`<button type="button" class="btn btn-danger px-4" data-bs-toggle="modal" data-bs-target="#modalEndSection" id="ES">End Section</button>`;
    else
        temps+=`<button type="button" class="btn btn-danger px-4" id="Submit" data-bs-toggle="modal" data-bs-target="#modalsubmit">Submit</button>`;
    if(num!=sectional_qnum[curr_sec][1])
        temps+=`<button type="button" class="btn btn-primary px-4" id="Next" onClick="nextclick()">Next</button>`;
    mcq.innerHTML=str;
    navbtn.innerHTML=temps;
    if(response[`Q${qnum})`]){
